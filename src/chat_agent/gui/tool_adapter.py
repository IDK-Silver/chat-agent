...
GUI_TASK_DEFINITION = ToolDefinition(
    ...
    description=(
        ...
        "'Send a message saying hello in the Line app'."
    ),
    parameters={
...
def create_gui_task(manager: GUIManager) -> Callable[..., str]:
    ...
    def gui_task(intent: str = "", **kwargs: Any) -> str:
        if not intent:
            return "Error: intent is required."
        try:
            result = manager.execute_task(intent)
        except Exception as e:
            logger.error("GUI task error: %s", e)
            return f"GUI task error: {e}"
        status = "SUCCESS" if result.success else "FAILED"
        return f"[GUI {status}] (steps: {result.steps_used})\n{result.summary}"
