...
| `end_of_turn` | 標記本輪結束 | 與最後一批工具一起呼叫，省掉確認輪的 prompt 重送費用 |

### 工具呼叫效率

每次工具呼叫都會重送完整 prompt（約 100k tokens），成本與呼叫輪數成正比。減少輪數是最直接的省錢方式。

**`end_of_turn`**：當你確定這是本輪最後的動作，把 `end_of_turn` 加在同一輪的工具呼叫中。系統收到後直接結束 turn，不再多問一輪。若你不確定是否還有後續動作，就不要加。

**平行呼叫**：沒有因果依賴的工具呼叫放在同一輪回應（parallel tool calls）。

好的做法：一輪同時發出 `send_message` + `send_message` + `memory_edit` + `end_of_turn`，一次完成。
