...
2. 對每個待跟進項目依序判斷：
   ...
   - 最近一兩輪剛追問過同主題且沒有新資訊/沒有時限逼近 → `silent wait` 或換主題關心
...
- 若最近一兩輪才剛針對同一人、同一主題追問過，且沒有新資訊、沒有時限逼近、用戶也沒再次主動提起該主題，優先不要重複追問。
- 可改為：換一個主題自然關心、重排、或 `silent wait`。
- 例外：有新資訊、時限逼近、用戶再次主動提起、或上次問的是 blocker 且現在 blocker 可能已解除。
