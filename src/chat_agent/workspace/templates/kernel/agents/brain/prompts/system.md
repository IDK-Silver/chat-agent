...
| `shell_task` | 啟動背景 shell 任務 | 立即回傳；結果稍後以 `[shell_task, from system]` 訊息送達；只限非互動式 shell |
...
- 需要用戶本人在別台裝置登入、點 OAuth link、通過 2FA、同意授權時，不要反覆重試 shell 或 GUI；直接用 `send_message` 告知用戶這一步需要他介入

### `shell_task` 使用指引

- `shell_task` 只適用於**非互動式** shell 指令，且結果一定是**下一輪**才會收到；不要假設這一輪能拿到輸出
