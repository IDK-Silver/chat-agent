...
補充（2026-03）：若某個 inbound turn 在單輪內已耗盡 **transient / transport-like** LLM retry，brain turn 仍失敗，該 inbound 會以 delayed message 形式重新入 queue，而不是直接丟失。此重入受 `app.turn_failure_requeue_limit` 與 `app.turn_failure_requeue_delay_seconds` 控制；它是「失敗 turn 結束後重新排隊」，不是中斷當前 turn。

不適用於：
- request-format / malformed payload
- provider 明確拒絕的 4xx
- context-length 類失敗

這些 deterministic failure 會直接 surface error 並 ack，不再 queue replay 舊事件。

若明確需要保留這些 inbound 以便人工修完設定或 provider 問題後重播，可將 `app.requeue_non_retryable_turn_failures: true` 打開。打開後，deterministic failure 也會共用同一組 `turn_failure_requeue_limit` / `turn_failure_requeue_delay_seconds` 進 delayed replay；預設仍為關閉。
...
- 既有 compact 機制（`preserve_turns`）與 token soft-limit 策略可處理
- `/compact` 命令已存在
