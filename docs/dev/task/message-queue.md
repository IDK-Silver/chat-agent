...
補充（2026-03）：若某個 inbound turn 在單輪內已耗盡 LLM transport retry，brain turn 仍失敗，該 inbound 會以 delayed message 形式重新入 queue，而不是直接丟失。此重入受 `app.turn_failure_requeue_limit` 與 `app.turn_failure_requeue_delay_seconds` 控制；它是「失敗 turn 結束後重新排隊」，不是中斷當前 turn。
