...
| tool-loop metadata round-trip | assistant tool history 必須保留 provider 回傳的 `tool_calls[].id` 與 `tool_calls[].function.index`，避免 Gemini-backed Ollama 第二輪因 history 降格而回 `400` | `src/chat_agent/llm/providers/ollama_native.py` + `src/chat_agent/llm/schema.py` | 維持 native `/api/chat` round-trip fidelity |
